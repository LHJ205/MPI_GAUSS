...
int main(int argc, char **argv)
{
	...
	{
		cout << endl;
		cout << "Whole running time （总共用时）= " << time2 - starttime << endl;
		cout << "Distribute data time（分发数据用时） = " << time1 - starttime << endl;
		cout << "Parallel compute time（并行计算用时） = " << time2 - time1 << endl;
	}
